...
public:
        ...
        backendId(bId), targetId(tId)
    {
        ...
        this->inputSize = Size(G_INPUT_SIZE, G_INPUT_SIZE);
...
/*     Mat preprocess(Mat img)
    ...
    {
        Mat blob;
        // O YOLOX do OpenCV Zoo espera a imagem redimensionada para o tamanho do modelo (640x640)
        // e com as cores trocadas (SwapRB = true).
        blob = dnn::blobFromImage(img, 1.0, Size(img.cols, img.rows), Scalar(0,0,0), true, false);
        return blob;
    }
   ...
   Mat infer(Mat srcimg)
    ...
    Mat postprocess(Mat outputs)
    {
        ...
        vector<int> keep;
        // Sub NMSBoxesBatched for NMSBoxes
        dnn::NMSBoxes(boxesXYWH, maxScores, this->confThreshold, this->nmsThreshold, keep);
...
pair<Mat, double> letterBox(Mat srcimg, Size targetSize = Size(G_INPUT_SIZE, G_INPUT_SIZE))
{
...
Mat visualize(Mat dets, Mat srcimg, double letterbox_scale, double fps = -1)
{
    ...
    if (fps > 0)
        putText(resImg, format("FPS: %.2f", fps), Size(10, 25), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(0, 0, 255), 1);
    ...
    for (int row = 0; row < dets.rows; row++)
    {
        ...
        rectangle(resImg, Point(x0, y0 + 1), Point(x0 + txtSize.width + 1, y0 + int(1.5 * txtSize.height)), Scalar(0, 255, 0), -1);
